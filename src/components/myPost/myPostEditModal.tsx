...
const MyPostEditModal: React.FC<{  onClose: any, editPost:any , oldText:string, oldId:string}> = ({  onClose, editPost, oldText, oldId }) => {
    const [text, setText] = useState(oldText);
    const handleAddPost = async () => {
        await editPost(oldId, text);
        onClose();
    }
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="w-full max-w-md h-100 p-6 bg-white border rounded-lg shadow-md overflow-hidden">
                <h3 className="mb-4 text-xl font-bold text-center">{"Edit Post"}</h3>
